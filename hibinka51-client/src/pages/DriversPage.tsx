...
import {
  ...
  TableHead, TableRow, Paper, Chip, MenuItem, Select,
...
type SortKey = "call_sign" | "phone" | "status" | "is_external";
...
const STATUS_COLORS: Record<DriverStatus, "success" | "warning" | "default"> = {
  READY: "success",
  BUSY: "warning",
  OFF_DUTY: "default",
};
...
export const DriversPage = () => {
  ...
  const reload = () => { setSkip(0); load(0); };

  useEffect(() => { reload(); }, [search, filterStatus, filterExternal, sortKey, sortDir]);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1000, mx: "auto" }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Button variant="contained" onClick={() => { setEditDriver(null); setModalOpen(true); }}>
          + Новый водитель
        </Button>
      </Stack>
                ...
                <TableRow key={d.id}>
                  ...
                  <TableCell>
                    <Chip label={d.is_external ? "Внешний" : "Свой"}
                      color={d.is_external ? "warning" : "primary"} size="small" variant="outlined" />
                  </TableCell>
                  <TableCell align="right">
                    <Stack direction="row" justifyContent="flex-end" spacing={1}>
                      <Button size="small" variant="contained" sx={{ minWidth: "unset", px: 1, py: 0.5 }}
                        onClick={() => { setEditDriver(d); setModalOpen(true); }}>
